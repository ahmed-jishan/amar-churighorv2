...
import { Search, Phone, MapPin, ShoppingBag, Mail } from 'lucide-react';
...
function buildCustomerKey(email: string | undefined, phone: string): string {
  // If email exists, use email|phone as unique key
  // Combines both so same email + different phone = different customer
  if (email && email.trim()) return `email:${email.trim().toLowerCase()}|phone:${phone}`;
  // Fallback: phone only
  return `phone:${phone}`;
}
...
export default function AdminCustomersPage() {
  ...
  const customers = useMemo(() => {
    ...
    orders.forEach(o => {
      ...
      const key = buildCustomerKey(email, o.customer.phone);
  ...
  const filtered = customers.filter(c =>
    !search || c.fullName.toLowerCase().includes(search.toLowerCase()) || c.phone.includes(search) || c.email.toLowerCase().includes(search.toLowerCase())
  );
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? Array(5).fill(0).map((_, i) => (
                ...
                <tr key={`${c.email}|${c.phone}`} className="border-b border-[#1f3334]/40 hover:bg-[#051a1b] transition">
                  <td className="p-4">
